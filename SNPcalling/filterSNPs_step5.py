...
allfiles = [i for i in os.listdir('.') if i.endswith('.step4.vcf')]
print 'Total %s files'%len(allfiles)
for i in allfiles:
    prefix = i.split('.')[0]
    new_fn = prefix + '.step5.vcf' 
    cmd = "bcftools view -i 'AF>0.01 & AF<0.99' %s >  %s"%(i, new_fn)
    jobfile = '%s.step5.slurm'%prefix
    f = open(jobfile, 'w')
